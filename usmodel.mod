...
parameters 
    ...
    cgy            ${\rho_{ga}}$               (long_name='Feedback technology on exogenous spending (rho_ga)')
    curvp          ${\varepsilon_p}$           (long_name='Curvature Kimball aggregator prices')
    constelab      ${\bar{l}}$                 (long_name='Steady state hours worked (l_bar)')
    constepinf     ${\bar{\pi}}$               (long_name='Steady state inflation rate (pi_bar)')
    constebeta     ${100(\beta^{-1}-1)}$       (long_name='Time preference rate in percent (100 * (beta^-1 - 1))')
    cmaw           ${\mu_w}$                   (long_name='Coefficient on MA term wage markup (mu_w)') 
    cmap           ${\mu_p}$                   (long_name='Coefficient on MA term price markup (mu_p)') 
    calfa          ${\alpha}$                  (long_name='Capital share (alpha)')
    czcap          ${\psi}$                    (long_name='Capacity utilization elasticity (psi)') 
    csadjcost      ${\varphi}$                 (long_name='Investment adjustment cost (varphi)')
    ctou           ${\delta}$                  (long_name='Depreciation rate of capital (delta)')
    csigma         ${\sigma_c}$                (long_name='Intertemporal elasticity of substitution (sigma_c)')
    chabb          ${h}$                       (long_name='Habit parameter (h)')
    ccs            ${d_4}$                     (long_name='Unused parameter')
    cinvs          ${d_3}$                     (long_name='Unused parameter')
    cfc            ${\phi_p}$                  (long_name='Share of fixed costs (phi_p)')
    cindw          ${\iota_w}$                 (long_name='Indexation of wages to past inflation (iota_w)')  
    cindp          ${\iota_p}$                 (long_name='Indexation of prices to past inflation (iota_p)')
    cprobw         ${\xi_w}$                   (long_name='Calvo parameter for wages (xi_w)')
    cprobp         ${\xi_p}$                   (long_name='Calvo parameter for prices (xi_p)')
    csigl          ${\sigma_l}$                (long_name='Frisch elasticity of labour supply (sigma_l)')
    clandaw        ${\lambda_w}$               (long_name='Steady state mark-up in the labor market (lambda_w)')
    crdpi          ${r_{\Delta \pi}}$          (long_name='Unused parameter')
    crpi           ${r_{\pi}}$                 (long_name='Taylor rule inflation feedback (r_pi)') 
    crdy           ${r_{\Delta y}}$            (long_name='Taylor rule output growth feedback (r_Delta y)') 
    cry            ${r_{y}}$                   (long_name='Taylor rule output level feedback (r_y)')
    crr            ${\rho}$                    (long_name='Interest rate persistence (rho)')
    crhoa          ${\rho_a}$                  (long_name='Persistence of the productivity shock (rho_a)')
    crhoas         ${d_2}$                     (long_name='Unused parameter')
    crhob          ${\rho_b}$                  (long_name='Persistence of the risk premium shock (rho_b)')
    crhog          ${\rho_g}$                  (long_name='Persistence of the spending shock (rho_g)')
    crhols         ${d_1}$                     (long_name='Unused parameter')
    crhoqs         ${\rho_i}$                  (long_name='Persistence of the investment technology shock (rho_i)')
    crhoms         ${\rho_r}$                  (long_name='Persistence of the monetary policy shock (rho_r)')
    crhopinf       ${\rho_p}$                  (long_name='Persistence of the price markup shock (rho_p)')
    crhow          ${\rho_w}$                  (long_name='Persistence of the wage markup shock (rho_w)')
    ctrend         ${\bar{\gamma}}$            (long_name='Trend growth rate (gamma_bar)')
